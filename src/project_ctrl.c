...
int Loadig() // 加载进度条
{
    ...
    if (ret != 0)
        printf("create pthread for loading_pic failed\n");
    ...
    if (ret != 0)
        printf("cancel loading_pic pailed\n");
    return 0;
}
...
int ShowBmpslp(int s_x, int s_y, const char *picName) // 渐进打开图片
{
    ...
    // 5.将转化后的像素写入到映射内存中（赋值）
    for (y = s_y; y < height + s_y && y < 480; y++) // 显示第一组
    {
        for (x = s_x; x < wide + s_x && x < 800; x++)
        {
            // LC显示图片的第一个像素点            图片的第一个像素点
    ...
    usleep(50000);
    for (y = s_y + 21; y < height + s_y && y < 480; y++) // 显示第二组
    {
        for (x = s_x; x < wide + s_x && x < 800; x++)
        {
            // LC显示图片的第一个像素点            图片的第一个像素点
