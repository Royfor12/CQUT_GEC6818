...
int recorder_main(const char* filename)
{
	//1.打开wav文件，用来存放录制好的音频数据
	int wav_fd = open(filename, O_CREAT|O_WRONLY|O_TRUNC, 0777);
	if (wav_fd == -1)
	{
		perror("open() error");
		exit(1);
...
void prepare_wav_params(wav_format *wav)
{
	...
	wav->data.data_size = LE_INT(DURATION_TIME * wav->format.byte_rate);	// 除了WAV格式头之外的音频数据大小
}
