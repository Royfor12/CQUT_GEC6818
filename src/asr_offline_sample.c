...
int run_asr(UserData *udata,const char* filename, char* str_result); //进行离线语法识别
...
*/
int run_asr(UserData *udata,const char* filename, char* str_result)
{
...
{
	const char *login_config    = "appid = 3619b03c"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
    ...
    ret = run_asr(&asr_data, filename, str_result);
    if (MSP_SUCCESS != ret) {
        printf("离线语法识别出错: %d \n", ret);
        goto exit;
    }
